...
export default function MainFloors() {
  ...
  const { data, goNextPage, goPrevPage, page, isFetching, setPage } = useFloors();
  ...
  const handleClickAddFavorites = (index: number) => {
    ...
    if (token) {
      ...
      toastApi.show("Login para favoritos", {
        type: "confirmation",
        duration: 5000,
  ...
  return (
    <div className="flex flex-col items-center  py-10 px-4">
      <div className="my-5 mb-10">
        <h2 style={{ fontFamily: "time new roman" }} className="text-center text-3xl font-bold text-green-700">
          Nuestros Productos
        </h2>
        ...
        <div className="flex justify-center items-center gap-6 my-5">
          <button
            ...
            className={`${baseStyle} ${active === "nuevos" ? activeStyle : inactiveStyle}`}
          >
            Nuevos Productos
          </button>
          ...
          <button
            ...
            className={`${baseStyle} ${active === "favoritos" ? activeStyle : inactiveStyle}`}
          >
            Favoritos
          </button>
          ...
          <select
            ...
            className={` ${baseStyle} ${active === "categorias" ? activeStyle : inactiveStyle}`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
        {data?.data.map((floor, i) => (
          <div className="bg-green-100 px-5 rounded-xl">
            <div className="my-3 flex justify-end" onClick={() => handleClickAddFavorites(i)} style={{ cursor: "pointer" }}>
              <motion.div
                whileTap={{ scale: 1.3 }}
                animate={{ scale: likedImages[i] ? [1, 1.3, 1] : 1 }}
                transition={{ duration: 0.3 }}
              >
                {likedImages[i] ? <FaHeart size={30} color="green" /> : <FaRegHeart size={30} color="gray" />}
              </motion.div>
            </div>
            <div
              key={i}
            ...
            {Array.from({ length: data.totalPages }, (_, i) => (
              <button
                ...
                className={`px-3 py-2 rounded-xl ${
                  page === i + 1 ? "bg-green-700 text-white" : "bg-gray-200 text-green-700 hover:bg-green-300"
                } transition`}
              >
                {i + 1}
              </button>
